...
public:
  DiagnosticsPublisher()
      : Node("diagnostics_publisher"), timer_count_(0), last_msg_time_(now()) {
    ...
    timer_ = create_wall_timer(
        100ms, std::bind(&DiagnosticsPublisher::timer_callback, this));

    // Timer to check for staleness
    staleness_timer_ = create_wall_timer(
        500ms,
        std::bind(&DiagnosticsPublisher::staleness_check_callback, this));
  }
